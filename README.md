...
A capstone-oriented consolidated toolkit (covering objectives, AI prompt journal, testing/iteration notes, references) is available at `docs/TOOLKIT.md`.
...
Run newly added repository unit tests:
```bash
cd services/user-service && go test ./internal/repository -v
cd services/product-service && go test ./internal/repository -v
cd services/order-service && go test ./internal/repository -v
```
